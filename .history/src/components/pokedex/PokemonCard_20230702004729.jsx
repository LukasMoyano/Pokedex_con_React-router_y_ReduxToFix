...
<article className="">
  ...
  {/* Sección NOMBRE */}
  <section>
    <h3 className="mx-auto items-center capitalize hover:uppercase text-center font-bold text-3xl my-16">
      {pokemon?.name}
    </h3>
  ...
  <div className="flex flex-wrap">
    {pokemon?.stats?.map((stat) => (
      <div className="flex flex-col items-center p-2 border border-gray-300 rounded-lg w-1/2 max-w-xs" key={stat.stat.name}>
        {/* Tipo de ataque */}
        <h6 className="px-1 bg-gray-800 text-white text-xs">{stat.stat.name}</h6>
        {/* Fuerza del ataque */}
        <span className="py-0.5 px-1 rounded bg-gray-500 text-white text-xs">{stat.base_stat}</span>
      </div>
    ))}
  </div>
</section>
