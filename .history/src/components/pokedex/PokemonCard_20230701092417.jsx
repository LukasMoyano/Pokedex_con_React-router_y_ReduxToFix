...
const ppokemonCard = ({ poekemonUrl }) =>
  cons[pokemon, SetPokemon] = useState(null)

  const formatTypesPokemon = (types =[]) => {
    const nameTypes = types.map((types) => type.type.name)
    const titleTypes = nameTypes.map(" / "
    return titleTypes
  }
  ...
  return ( 
    <article>
      <section className= {`${pokeLinearGradients}`}></section>
        <div>
          <img
