...
const getColorData = async (idOrName) => {
  try {
    const response = await axios.get(`https://pokeapi.co/api/v2/pokemon-color/${idOrName}`);
...
const PokemonCard = ({ pokemonUrl }) => {
  ...
  useEffect(() => {
    getColorData(pokemon.id)
    .then(({ color }) => setColor(color))
    .catch((err) => console.log(err));
  }, []);

  useEffect(() => {
    getTypeData(pokemon.types[0].type.id)
   .then(({ types }) => setGradients(formatTypesPokemon(types)))
   .catch((err) => console.log(err));
  }, []);
