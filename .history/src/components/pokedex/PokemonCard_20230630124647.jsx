...
const getColorData = async (id_Or_Name) => {
  try {
    const response = await axios.get(`https://pokeapi.co/api/v2/pokemon-color/${id_Or_Name}`);
...
const getTypeData = async (id_Or_Name) => {
  try {
    const response = await axios.get(`https://pokeapi.co/api/v2/type/${id_Or_Name}`);
    return response.data;
  } catch (error) {
    console.log(error);
    throw error;
