...
fetch('https://pokeapi.co/api/v2/pokemon/?limit=40')
  ...
  .then(data => {
    // Obtén la URL del primer Pokémon de la lista
    const pokemonUrl = data.results[0].url;

    // Realiza una solicitud a la URL del primer Pokémon para obtener sus detalles
    return fetch(pokemonUrl);
  })
  .then(response => response.json())
  .then(pokemonData => {
    // Aquí puedes asignar los valores del Pokémon a una variable o utilizarlos directamente
