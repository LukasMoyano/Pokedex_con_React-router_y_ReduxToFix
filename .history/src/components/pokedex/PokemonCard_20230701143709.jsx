...
const PokemonCard = ({ pokemonUrl }) => {
  ...
  return (
    <article className="image-container">
      <section className={`relative ${pokeLinearGradients[pokemon?.types[0]?.type?.name]}`}>
        <div classeName="absolute">
          <img src={pokemon?.sprites?.other["official-artwork"].front_default} alt={pokemon?.name} style={{ maxWidth: '100%', height: 'auto' }} />
        </div>
      </section>
