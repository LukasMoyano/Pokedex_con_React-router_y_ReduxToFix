...
<article className="flex flex-col mb-4">
  <section className={`relative h-40 ${pokeLinearGradients[pokemon?.types[0]?.type?.name]}`}>
    <div className="px-5 -botton-2 w-full h-full">
      <img className="w-full h-full object-contain" src={pokemon?.sprites?.other["official-artwork"].front_default} alt={pokemon?.name} />
    </div>
  </section>
  {/* Sección NOMBRE */}
  <section className={`${window.innerHeight <= 400 ? 'flex flex-col' : 'flex'}`}>
    <h3 className={`${window.innerHeight <= 400 ? 'text-left' : 'mx-auto'} items-center capitalize hover:uppercase text-center font-bold text-3xl my-5`}>
      {pokemon?.name}
    </h3>


    {/* TYPE */}
    <span className=" mx-auto items-center text-center text-center text-sm">Type</span>
  ...
  <section className="whitespace-normal">
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      {pokemon?.stats?.map((stat) => (
        <div className="flex flex-col items-center" key={stat.stat.name}>
          {/* Tipo de ataque */}
          <h6 className="text-center">{stat.stat.name}</h6>
          {/* Fuerza del ataque */}
